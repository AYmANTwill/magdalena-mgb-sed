...
*Revised 2026-08-19 by the repository-reorganization pass
(`docs/agents/PROMPT_repo_reorganization.md`; journals `journal_reorg-triage.md`,
`journal_reorg-coherence.md`, `journal_reorg-notebooks.md`): documents **54–59** added to §3, the
C4.3 and C5 answers in §4 corrected against their owning docs now that **C4.3 has run and C5 is
complete**, §7's defect register reconciled, and the new top-level **`README.md`** and
**`docs/archive/README.md`** wired in. Every correction is struck-through-and-annotated, never
overwritten.*

> **START HERE INSTEAD, if you are new.** The repository root now carries a **`README.md`** written
> as thesis front matter: the project in one paragraph, a six-document reading order, the narrative
> arc, a glossary, a reproduction quick-start with its four gates, and the folder map. This index is
> the *reference* map — every document with a status and a successor. Read the README first, then
> come here for the detail.
...
ceiling rather than at a target; **Phase C** (sediment) is ~~in flight~~ → **COMPLETE
(2026-08-19)**: the SSC quality gate (C1) and the model-free observed ENSO contrast (C2) are done;
the MUSLE drivers and engine (C3) are built but **C3 is OPEN** (`docs/37`, four amendments deep) —
which is the LS-**level** validation, a **permanent limitation, not a blocker**: Phase C completed
around it because the ENSO-contrast result is LS-invariant (`docs/37`'s 2026-08-19 note);
~~calibration + the ENSO experiment (C4, C5) remain — with **C4.3, the calibration search itself,
formally BLOCKED** until the LS level lands~~ → **C4.3 has RUN and its verdict is
`RAILED / EXPLORATORY, not adopted` (`docs/55`), and C5 has REPRODUCED the observed ENSO contrast
at 18/18 stations, median rate ratio 3.05× (`docs/56`)**. Three closing limits are now numbered
rather than asserted: the flux gauge set cannot grow past ~18 (`docs/57`), the last rainfall lever
is bounded at ≤ +0.006 r (`docs/58`), and an independent second implementation of the same method
on the same basin reached the same non-identifiability conclusion (`docs/59`). **The two-level
result must not be collapsed into one: the absolute level is non-identifiable and the daily
prediction is rainfall-limited; the ENSO *contrast* is the finding, and it survives precisely
because it is a ratio in which every unidentifiable multiplier cancels exactly.** The project's governing discipline is **pre-registration**: thresholds and
...
> **UPDATED 2026-08-19.** The canonical reading order now lives in the repository's
> **`README.md`** §2 — six documents, ending on the two that carry the answer (`docs/55`
> then `docs/56`) and the three that bound it (`docs/57`–`59`). Read that one. The table below
> is kept because it is cited elsewhere, with two corrections applied in place:
> **entry 2 (`docs/21`) is archived in place as a HISTORICAL snapshot (2026-08-03)** — take its
> *reasoning*, never its *status*; and **entries 4–5 no longer describe the current state**,
> because `docs/30`/`docs/31` are plans for a phase that has since completed. **Nothing in the
> five below mentions the study's headline result**, which is `docs/56`; that alone is the
> reason the README's order supersedes it.
...
| 47 | c4_entry_verdict | **the C4.3 entry verdict, as recorded 2026-08-11: `C4.3-BLOCKED-UNTIL-LS-LANDS`.** The α box is denominated in an unresolved unit; the objective is monotone across the whole box; in-box `F_report` −0.305 … −0.350 | **HISTORICAL — SUPERSEDED AS A LIVE BLOCK by its own §10 (2026-08-19): C4.3 HAS RUN, `docs/55` owns the verdict.** Retained as the record of the entry contract; §10.1 discharges all four §9.2 blockers one by one (ACT 1/2, `docs/35` §9.5, `docs/46` amd 3/4) | quote `docs/55` for any C4.3 number, never this file |
...
> **The next free number is 60** (checked 2026-08-19 against `ls docs/*.md`; 54–59 are assigned
> below).

### Phase C — the results and the limits (docs 54–59)

| # | title | subject | status | superseded by |
|---|---|---|---|---|
| 54 | c3_1_closure_and_c4_entry_status | a pointer-only status snapshot of the C3.1 closeout and the C4 entry condition; adds no new numbers | **HISTORICAL** (2026-08-12, archived in place) — its four "surviving blockers" and its `140 passed` test count are both overtaken | 55 (C4.3 ran) · `docs/37` A3 (the C3.1 enactment it points at) · `progress_map.html` |
| 55 | c43_verdict | **the C4.3 deliverable: the sediment calibration verdict.** `RAILED / EXPLORATORY, NOT adopted` — the in-box optimum sits on the box floor (α = 2.0, median `KGE_ln` **−0.118** on estimator (a), **+0.139** on (b): same sign, so *not* indeterminate), and the unconstrained optimum wants α ≈ **0.48**, below the floor, which is the registered signature of mild upstream over-production — *a diagnosis, not a value to adopt*. §5 states plainly why a high KGE was never attainable | **LIVE** — the authority on the C4.3 outcome | — |
| 56 | c5_enso_application | **the C5 deliverable, and the study's headline result: the model REPRODUCES the observed ENSO sediment contrast.** 18/18 stations, median rate ratio **3.05×** (1.62–4.85), geo-mean 3.06, robust in all six sensitivity cells (β ∈ {0.45, 0.56, 0.65} × two window pairs). §2 is the load-bearing argument: the within-station ratio is **invariant to α and to the LS level**, so the C4.3 railing does not touch it. The reference observed estimator is named **(b)**, and the modelled median sits at the **lower edge** of the observed 3–5× band | **LIVE** — the headline | — |
| 57 | b5_gauge_expansion | background task B5: all 46 unmapped SSC stations geocoded, 43 in-basin — and **zero of the 43 have any discharge record**, so the flux-calibration gauge set **cannot grow past ~18**. A physical limit of the monitoring network, not a processing gap. §3 adds the weaker concentration-only contrast (11/16, median 1.38) that the recovery *does* unlock | **LIVE** — owns the gauge-count limit | — |
| 58 | rainfall_ceiling_bound | the last surviving hypothesis on the r ≈ 0.57 ceiling (the 139 residual rain-selective gauges), **bounded rather than tested**: even a perfect repair buys **≤ +0.006 r**, because the merge helps at 10–30 km isolation (+0.023 over 57 % of the basin) and *hurts* beyond 30 km (−0.043 over 17 %). **The ceiling is structural.** *"Not worth testing" is a different and stronger statement than "not yet tested."* | **LIVE** — owns the ceiling bound | — |
| 59 | cross_implementation_comparison | the comparison against a second, independent implementation of MGB-SED on this basin (**R2**, `github.com/yben409/…` @ `d055561`, 2026-08-03). **Verdict: an INDEPENDENT IMPLEMENTATION, but NOT INDEPENDENT DATA** — and the second half governs how the comparison may be used. R2 independently documents its own collinearity and found a defect in its own concentration that invalidated its own published sediment score. Three of this project's premises about R2 were wrong and are corrected in the body | **LIVE** — but a **snapshot**: every R2 number must be quoted with the commit `d055561` and the date | — |

> **Citing docs/59.** Its §0 pins every R2 number to one commit and warns that R2's own numbers are
> mid-recalibration. The archive that keeps it reproducible is
> `data/raw/refs/yben409_sediment_repo.bundle` plus the 306 MB
> `magdalena_share_for_colleague.zip` at the repository root (gitignored, **not regenerable**, 20/20
> member hashes verified against `data/raw/colleague_share/input_hashes.txt`).
...
| **Does the MODEL reproduce the ENSO contrast?** | **Yes — and this is the study's headline result.** `docs/56`: run on the frozen sediment configuration over the strictly out-of-sample ENSO windows, the model gives **La Niña > El Niño at 18 of 18 usable stations, median rate ratio 3.05×** (range 1.62–4.85, geo-mean 3.06), against the model-free observed **22/22, median ~3–9×**. The modelled median sits at the **lower edge** of the observed 3–5× primary band and matches the smoother rating-based estimator **(b)** — stated that way deliberately, so the model is not compared against whichever observed estimator flatters it. Direction holds in **all six** sensitivity cells (β ∈ {0.45, 0.56, 0.65} × primary/secondary windows); the secondary ONI-peak windows *sharpen* it (3.78 → 4.92 → 5.90). **Why it survives C4.3's railing:** the within-station wet/dry ratio is **invariant to α and to the LS level** — both are static multipliers that cancel exactly — so a non-identifiable *level* does not touch a *ratio*. It is also why a poor daily KGE and a right seasonal contrast are consistent rather than contradictory. **What it is NOT:** a validated absolute flux, and it does not rescue the C4.3 level. | **`docs/56`** (the result, §2 the invariance argument, §4 what it is and is not) · `docs/34` (the observed target) · `docs/55` (why the level still fails) |
| **Can the gauge set be made bigger?** | **No, and the limit is physical.** `docs/57`: all **46** SSC stations previously excluded for "no coordinates" were found in the IDEAM Catálogo Nacional de Estaciones and geocoded; **43 fall inside the basin** and 44 carry SSC records. But sediment **flux** needs same-code discharge, and **0 of the 43** have any discharge record in `discharge_daily` *or* anywhere in the raw IDEAM download. They are sediment-only sampling points — **there is nothing to fetch**. So the flux-calibration set **cannot grow past ~18**, and the nearest discharge gauge is typically 10–35 km away on a different drainage area. What the recovery *does* buy is a broader but coarser check: the concentration-only contrast, 16 qualifying stations (8 of them newly recovered), **11 of 16** wet > dry, median 1.38 — same direction, much weaker. | **`docs/57`** (§2 the binding constraint, §4 the honest bottom line) · `docs/32` §R6 |
| **Can the r ≈ 0.57 ceiling be lifted?** | **No — and this is now bounded, not merely asserted.** `docs/58`: the one untested upstream hypothesis (repair the 139 gauges still reporting rain-selectively, then adopt the CHIRPS merge) is answered by an **arithmetic upper bound** on the merge's own LOOCV numbers, without running anything. The merge is **signed by gauge isolation**: it changes nothing below 10 km (25.8 % of the basin), helps by **+0.023** at 10–30 km (57.1 %), and **hurts by −0.043** beyond 30 km (17.1 %). Area-weighted, **Δr = +0.0058**. Two reasons that is optimistic: it assumes the repair fully succeeds *and* the merge is adopted everywhere, and it is a *precipitation-field* gain, while discharge r integrates and smooths it — so the hydrology gain is **≤ +0.006**, not equal to it. **The ceiling is the information content of the observations, not a solvable processing problem.** | **`docs/58`** · `docs/22` §4.7 (the ceiling itself) · `docs/18` §15.5 |
| **Has anyone else modelled this basin, and does it agree?** | **Yes, and the distinction is the whole point: an INDEPENDENT IMPLEMENTATION, NOT INDEPENDENT DATA.** `docs/59` compares against R2 (`github.com/yben409/simulating-suspended-sediment-transport` @ `d055561`, 2026-08-03) — a separate codebase on the same basin, holding out both ENSO windows by name and citing the same published bar. It reaches the **same non-identifiability conclusion**, documenting its own collinearity in its own shipped output. But it draws on largely the **same IDEAM holdings**, so agreement is *not* independent corroboration of the data. Three of this project's premises about R2 turned out to be wrong and are corrected in the body rather than a footnote; R2 also found and reported a defect in its own simulated concentration that invalidated its own published sediment score. **Every R2 number must be quoted with the commit and the date** — R2's own manifest says a recalibration was mid-flight. | **`docs/59`** (§0 the pin, §10 what it owes other docs) |
...
> **See also `docs/archive/README.md`** (added 2026-08-19): the archive policy and the register of
> every document archived *in place* with its successor. Seventeen numbered and unnumbered
> documents are archived in place rather than moved, because relocating them would have broken
> **352 inbound citations**, 125 of them from live documents and several from inside frozen
> pre-registrations that may only be changed through their own amendment slots. `docs/agents/`
> itself is left where it is and declared non-authoritative by `docs/agents/README.md`.
...
|---|---|---|
| LS2D build (Desmet & Govers, 30.2 M cells at 90 m, median LS 12.8 vs ~~published 2–10~~ → **the since-RETIRED, UNCITED "2–10" band** — reported, not adjusted) | `journal_c31-ls2d.md` + tracker | **yes** — `docs/37` (the C3 closure and its amendments) owns the LS decision; the 2–10 band is retired and *passes and fails nothing* |
...
| Sediment engine build + test/mass-ledger evidence (~~82/82 tests~~ → **82/82 at build time; the suite is now 154 passed, measured 2026-08-19**; ledger closed exactly) | `journal_c34-sediment-engine.md` + tracker | **yes** — `docs/37` and its amendments carry the engine record |
...
## 7 — Known documentation defects (as of 2026-08-11; items 7–11 added 2026-08-12; **reconciled 2026-08-19**)

> **RECONCILED 2026-08-19 by the reorganization pass.** A corpus-wide sweep classified 193 hits of
> the kill list and the canonical numbers and found **133 that read as current claims**; they are
> tabulated with file, line and the applied fix in **`docs/agents/journal_reorg-coherence.md`**.
> Against the list below: **item 4 is FIXED** (`README.md` was rewritten from scratch — it is now
> the project's front matter, and the "2015–2016 or 2017, to be confirmed" line is gone).
> **Item 5 is FIXED** where it read as current — the "Phase C blocked on mainstem SSC" phrasing is
> superseded by Phase C being COMPLETE, and the affected older docs are archived in place with a
> banner. **Item 9 is FIXED** — `CLAUDE.md`'s Phase status now reads *Phase C: COMPLETE*, with C4.3
> RAN → `docs/55` and C5 COMPLETE → `docs/56`. **Item 11's structural lesson held exactly as
> predicted**: the §3 table was again the stalest thing in this file, six documents behind
> (54–59), and is rebuilt above. **Items 1, 2, 3, 6, 7, 8 and 10 remain open** — 2 and 3 need a
> dated amendment by a doc owner rather than an editorial pass, 7 needs an amendment inside a
> frozen document, and 1/6/8/10 are stale-by-filename or stale-in-an-archived-doc and are covered
> by the archive banners.
>
> **One defect found by this pass and worth recording here**, because it was invisible to every
> earlier audit: `scripts/build_deck.py` referenced **12 notebook figures by a filename encoding a
> cell index**, and regenerating notebooks 10–14 shifted every one of those indices by +1. Because
> the deck builder degrades silently — it stamps a red `[missing figure: …]` box and carries on —
> the deck **built successfully while illustrated with figures that no longer corresponded to the
> notebooks they claimed to show**. All 12 were re-mapped and proved byte-identical against the
> previous build; three further deck figures turned out to have **no producer in the repository at
> all** and were being copied in by hand, which `scripts/generate_report_figures.py` now does
> reproducibly.
