...
> **Also annotated below, in place:** the intro's, §1's and §5's *"55 of 294 stations"* (the intro's and §5's notes were added 2026-08-19; on 2026-08-12 only §1's was) (this notebook's own executed output already prints **70**; the repair finished at **153**, `docs/18` §10.2), §3's ERA5 window, and §5's limitations 3 and 4.
>
...
1. **Is the conventional gauge dataset even clean?** Section 1 finds and repairs a defect that
   invalidated the earlier version of this notebook: ~~55~~ **70** stations had their dry days
   deleted. (RETIRED / superseded — note added 2026-08-19: **55** was the first pass, **70** is what
   §1's executed cell actually prints, and a second detector took the repair to **153** stations and
   **240,158** inferred-dry station-days, `docs/18` §10.2. The 55 is shown, not quoted as current.)
...
> **⚠ TWO PROPERTIES OF THIS SECTION'S ESTIMATOR — note added 2026-08-19; the text above and the numbers below are unchanged.**
>
> 1. **This section re-implements the interpolator inline, unguarded.** The cell below picks neighbours with a plain `np.argsort` on distance and does **not** merge co-located gauges. Three pairs in this same inventory sit at **0.000000 m** (notebook 11's co-located-gauge classification table prints them), so their distances **tie**, and the neighbour set at those ties is resolved by **column order** — the exact defect `docs/23` §11 fixed by lexsorting on (distance, gauge code) in `src/idw_forcing.py`, where shuffling gauge columns had moved up to **83 minibacias by up to 20.5 mm/day**. The **registered** leave-one-out statistic is notebook 11 §6's order-invariant **median daily r 0.429** — the LOOCV gate in `docs/18` §15.1 — and §4's **0.41** is a different estimator on a different gauge file. They are **not** interchangeable.
> 2. **Different gauge file.** §3 and §4 read `precip_gauges_daily_qc.csv`, the **70-station** repair, while notebook 11 and the adopted v2 forcing read the **153-station** `precip_gauges_daily_qc_v2.csv`. `docs/16` §4.3 measures how much the reference matters: CHIRPS volume bias moved **−13.6 → −7.6 → −5.8 %** across the pre-repair / 55-station / 70-station references, and the 153-station reference was **never measured**. This section's *verdict* — both fields damp extremes by essentially the same amount — does not turn on that. The individual numbers do.""")
...
> **⚠ SUPERSEDED COUNTS — note added 2026-08-19; the verdict above is unchanged.** ~~55 of 294 stations~~ → **70**, which is what §1's executed cell prints (*"flagged 70 / 294 stations (24 %)"*), and the repair then finished at **153 stations / 240,158 inferred-dry station-days** (`src/repair_precip_selectivity.py`, `docs/18` §10.2). ~~9,000-12,000 mm/yr~~ → the live output reads **3,863 → 1,794 mm/yr** for the flagged stations, against **2,034** for the healthy ones. Both figures are RETIRED / superseded — shown, not quoted as current. Neither the diagnosis nor this verdict changes: the defect was real, and the repair moved the totals into range.
...
>
> **⚠ THAT ROUTE IS NOW BOUNDED AND CLOSED — note added 2026-08-19.** ~~the only remaining upstream route~~ → **quantified, and negligible.** `docs/58` computed the route's *perfect-case* upper bound arithmetically, running nothing: applying `docs/18` §15.2's isolation-banded LOOCV deltas (**0.000** below 10 km, **+0.023** in the 10-30 km blend band, **−0.043** beyond 30 km) to notebook 11's own v2 area shares (**25.8 / 57.1 / 17.1 %**) gives **Δr = +0.0058 area-weighted** — the precipitation field's daily *r* would move ~0.57 → **~0.576**. Repairing the 139 stations cannot lift the ceiling: it is **structural**, not a pending repair. Owning doc: **`docs/58`**.""")
